...
class BayesianGhostBusterAgent(VacuumAgent):
    def __init__(self, log):
        ...
        self.path = []
        self.num_probes = 0
        # end_action is used to implement a behavior like 
        #  "go to (2,3) then PROBE" -- PROBE is the end_action
    ...
    #################################
    # World model always gives us a dictionary 
    #     square -> probability ghost is at square
    # Bust strategy is to bust at the square with the highest probability of a ghost,
    #  provided that probability exceeds the BUST_THRESHOLD
    # If there is no such square (all probabilities are under BUST_THRESHOLD), then 
    #  choose an unprobed square with highest ghost probability, and go probe there
    # If there are no unprobed square, go to some square that has been (unsuccessfully)
    #  busted, and probe there
    
    def bust_strategy(self):
        # sorted_coords are sorted in descending order of ghost probability
        sorted_coords = self.world_model.sorted_coords()
        busted_coords = self.world_model.busts
        probed_coords = self.world_model.probes
        unbusted = [sc for sc in sorted_coords if not (sc[0] in busted_coords)]
        if len(unbusted) > 0 and (unbusted[0][1] > BUST_THRESHOLD):
            self.log(f"GOING TO BUST at {unbusted[0][0]}, probability is " + "{:.2f}".format(unbusted[0][1]))
            return self.bust_at(unbusted[0][0])
        unprobed = [sc for sc in sorted_coords if not (sc[0] in probed_coords)]
        if len(unprobed) > 0:
    ...
    #################################################################
    #  Everything below is traversing path.   Easier than previous
    #  labs because there are no walls
